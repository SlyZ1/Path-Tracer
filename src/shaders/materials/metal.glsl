...
void metal(inout RaycastData data, bool inVolume){
    ...
    if (!inVolume){
        ...
        if (shadow_hit(light, lightRay) > 0){
            ...
            ray.radiance += clamp(getMuellerCoeff(ray, 0, 0) * f_r * weight * NdotL / pdirect, 0.0, CLAMP_VAL) * Le;
        }
    }
    ...
    vec3 L = reflect(-V, H);
    applyDepolarizer(ray, weight_VNDF_reflect(fresnelParams, N, V, L, alpha));
    ray.dir = L;
