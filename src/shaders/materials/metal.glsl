...
float p_GGX_reflect(vec3 normal, vec3 l, vec3 v, float alpha, bool simplify){
...
vec3 cookTorranceMetals(Hit hit, vec3 viewDir, vec3 lightDir, float alpha, bool simplify){
...
void metal(inout RaycastData data){
    ...
    if (shadow_hit(light, lightRay) > 0){
        float pGGX = p_GGX_reflect(hit.normal, lightDir, viewDir, alpha, false);
        float weight = computeWeight(pdirect, pGGX);

        vec3 f_r = cookTorranceMetals(hit, viewDir, lightDir, alpha, false);
        float NdotL = max(dot(hit.normal, lightDir), 0);
    ...
    float pGGX = p_GGX_reflect(hit.normal, GGXDir, viewDir, alpha, false);
    float NdotL = max(0, dot(GGXDir, hit.normal));
    vec3 f_r = cookTorranceMetals(hit, viewDir, GGXDir, alpha, false);
